...
int timeup(int, int);
void dspt(int, int);
void msg(int, int, int);

    //Startup Message

void msg(int start, int alarm, int set){
    ...
    char conv[38] = {};
    int c, x, y;
    char startmessage[] = {"time clock by_2032574     "};
    ...
    if(start == 1){
        mlgth = strlen(startmessage);
        for(x=0; x<mlgth; x++){

            for(c=0; c<sizeof(ascii); c++){

                if(startmessage[x] == ascii[c]){
    ...
    if (alarm == 1){
        ...
        mlgth = strlen(alarmmessage);

        for(x=0; x<mlgth; x++){

            for(c=0; c<sizeof(ascii); c++){

                if(alarmmessage[x] == ascii[c]){
    ...
    if (set== 1){
        ...
        mlgth = strlen(setmessage);

        for(x=0; x<mlgth; x++){

            for(c=0; c<sizeof(ascii); c++){

                if(setmessage[x] == ascii[c]){
...
void dspt(int hour, int minute){
    ...
    if((PIND == 0x01) || (PIND == 0x03)) || (PIND == 0x0B)) || (PIND == 0x13)){
